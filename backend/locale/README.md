...
```bash
docker compose -f local.yml run --rm app python manage.py makemessages -all --no-location
```
...
```bash
docker compose -f local.yml run --rm app python manage.py compilemessages
```
